...
from data.image_folder import make_dataset_category, make_dataset_test
...
class AlignedCategoryDataset(BaseDataset):
    def initialize(self, opt):
        self.opt = opt
        self.root = opt.dataroot
        self.diction={}
        self.textfile_path = './ACGPN_train/CategoryData.txt'
        ### input A (label maps)
        if opt.isTrain or opt.use_encoded_image:
            dir_A = '_A' if self.opt.label_nc == 0 else '_label'
            self.dir_A = os.path.join(opt.dataroot, opt.phase + dir_A)
            self.A_paths = sorted(make_dataset_category(self.dir_A, self.textfile_path))
            self.AR_paths = make_dataset_category(self.dir_A, self.textfile_path)
        ...
        self.B_paths = sorted(make_dataset_category(self.dir_B, self.textfile_path))
        self.BR_paths = sorted(make_dataset_category(self.dir_B, self.textfile_path))
        ...
        if opt.isTrain or opt.use_encoded_image:
            ...
            self.E_paths = sorted(make_dataset_category(self.dir_E, self.textfile_path))
            self.ER_paths = make_dataset_category(self.dir_E, self.textfile_path)

        ### input M (masks)
        if opt.isTrain or opt.use_encoded_image:
            dir_M = '_mask'
            self.dir_M = os.path.join(opt.dataroot, opt.phase + dir_M)
            self.M_paths = sorted(make_dataset_category(self.dir_M, self.textfile_path))
            self.MR_paths = make_dataset_category(self.dir_M, self.textfile_path)

        ### input MC(color_masks)
        if opt.isTrain or opt.use_encoded_image:
            dir_MC = '_colormask'
            self.dir_MC = os.path.join(opt.dataroot, opt.phase + dir_MC)
            self.MC_paths = sorted(make_dataset_category(self.dir_MC, self.textfile_path))
            self.MCR_paths = make_dataset_category(self.dir_MC, self.textfile_path)
        ### input C(color)
        if opt.isTrain or opt.use_encoded_image:
            dir_C = '_color'
            self.dir_C = os.path.join(opt.dataroot, opt.phase + dir_C)
            self.C_paths = sorted(make_dataset_category(self.dir_C, self.textfile_path))
            self.CR_paths = make_dataset_category(self.dir_C, self.textfile_path)
